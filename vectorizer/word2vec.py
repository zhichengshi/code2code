...
import math
from config import BATCH_SIZE,NUM_FEATURES,HIDDEN_NODES
def init_net(
        vocabulary_size,batch_size=BATCH_SIZE, num_feats=NUM_FEATURES, hidden_size=HIDDEN_NODES,
):
    ...
    with tf.name_scope('network'):
        ...
        with tf.name_scope('inputs'):
            ...
            embeddings = tf.Variable(
                tf.random_uniform([vocabulary_size, num_feats]), name='embeddings'
            )

            embed = tf.nn.embedding_lookup(embeddings, inputs)
            onehot_labels = tf.one_hot(labels, vocabulary_size, dtype=tf.float32)
        ...
        with tf.name_scope('softmax'):
            ...
            biases = tf.Variable(
                tf.zeros((vocabulary_size,), name='biases')
            )
